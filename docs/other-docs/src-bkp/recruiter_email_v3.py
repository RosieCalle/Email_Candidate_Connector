...
def gmail_authenticate():
    ...
    TODO: Rocio, learn about the OAuth2.0 protocol and how to use it to authenticate users in web applications.

    The error you're encountering, invalid_grant: Token has been expired or revoked, typically occurs when the OAuth 2.0 token
     used for authentication has either expired or has been revoked. This can happen for a few reasons:

    1) The token has expired: OAuth 2.0 tokens have a limited lifetime. Once expired, they need to be refreshed. 
         However, if the token is not refreshed in time, it becomes invalid.
    2) The token has been revoked: If a user revokes access to your application or if the token 
        is otherwise invalidated (e.g., due to security concerns), you will encounter this error.
    3) Misconfiguration or incorrect usage: Ensure that your application is correctly configured 
        in the Google Cloud Console and that you're using the correct credentials and scopes.
    
    To resolve this issue, you can try the following steps:

    1) Delete the existing token: If you have a token.pickle file in your project directory, delete it. 
        This file stores the user's access and refresh tokens. Deleting it will force the application to go 
        through the authentication flow again, generating a new token.
    2) Re-authenticate: Run your script again. It should prompt you to go through the OAuth 2.0 flow,
        where you'll need to authorize the application again. This will generate a new token.
    3) Check your application's configuration: Ensure that your application is correctly set up in the Google Cloud Console. 
        Verify that you're using the correct client_id and client_secret in your credentials.json file. Also, make sure that the scopes you're requesting match those configured in the Google Cloud Console for your application.
    4) Ensure your application is not exceeding quota limits: If your application is making too many requests 
        in a short period, it might be temporarily blocked. Check the Google Cloud Console for any quota-related issues.
    
    If you continue to experience issues after trying these steps, you may need to review the Google Cloud Console's logs for more detailed error messages or contact Google's support for further
    """
...
def process_message(service, message):
    """
    Processes a single message by extracting its content, attachments, and other details.

    Parameters:
    - service: The authenticated Gmail API service.
    - message: The message to be processed.

    Returns:
    - tuple: A tuple containing the email content, a list of attachments, and a status indicating success or failure.

    TODO: Verify that this works: 

    This modified process_message function now includes error handling for UnicodeEncodeError and other exceptions. 
    When writing the email content to a file, it specifies the encoding as 'utf-8' to handle Unicode characters properly.
      This should resolve the UnicodeEncodeError you were encountering. 
      
    Additionally, the function now returns a tuple containing the email content, a list of attachments, and a status 
    indicating success or failure, which can be used by the calling function to determine the next steps.
    """
    try:
        msg = service.users().messages().get(userId='me', id=message['id'], format='full').execute()
        payload = msg['payload']
        ...
        # Return a success status along with the processed data
        return email_content, attachments, "Success"
    except UnicodeEncodeError as e:
        print(f"UnicodeEncodeError while processing message ID {message['id']}: {e}")
        return "", [], "Failure"
    except Exception as e:
...
def save_downloaded_message_ids(message_ids, folder_path):
    """
    Incrementally saves downloaded message IDs to a file within a specified folder.

    The function generates a datetime-stamped filename for the IDs file, ensuring
    that each file has a unique name based on when it was created. This is useful for
    tracking and managing the files, especially when processing batches of emails.

    Parameters:
    - message_ids (list): A list of message IDs to be saved.
    - folder_path (str): The path to the folder where the IDs file will be saved.

    Returns:
    - str: "Success" if the operation was successful, "Failure" otherwise.

    Raises:
    - Exception: If an error occurs while generating the datetime-stamped filename,
                 creating the file, or writing the message IDs to the file.
    """
    try:
        # Generate a datetime-stamped filename for the IDs file
        timestamp = time.strftime("%Y%m%d-%H%M%S")
        ids_file_name = f"downloaded_message_ids_{timestamp}.txt"
        ids_file_path = os.path.join(folder_path, ids_file_name)

        # Open the file in append mode to incrementally save message IDs
        with open(ids_file_path, 'a', encoding='utf-8') as file:
            for msg_id in message_ids:
                file.write(f"{msg_id}\n")
        print(f"Saved downloaded message IDs to {ids_file_path}")
        return "Function save_downloaded_message_ids SUCCEEDED!"
    except Exception as e:
        print(f"Error saving downloaded message IDs to {ids_file_path}: {e}")
        return "Function save_downloaded_message_ids FAILED."
...
def main():
    ...
    # TODO: Verify that the message IDs are already ACTUALLY downloaded. 
